    ...
    public static class ImportService
    {
        ...
        public static ImportResult ImportExcel(string path)
        {
            ...
            for (int r = bestHeaderRow + 1; r <= lastRow; r++)
            {
                ...
                for (int c = 1; c <= lastCol; c++)
                    if (!string.IsNullOrWhiteSpace(bestWs.Cell(r, c).GetString())) { empty = false; break; }
            ...
            result.Rows = result.Rows
                .OrderBy(x =>
                {
                    ...
                    int n; return int.TryParse(d, out n) ? n : int.MaxValue;
                })
                .ThenBy(x => x.Muelle)
                .ToList();
        ...
        private static (int hits, int headerRow, Dictionary<string,int> map) DetectHeaders(IXLWorksheet ws)
        {
            ...
            for (int r = 1; r <= maxRow; r++)
            {
                ...
                if (hits > bestHits) { bestHits = hits; headerRow = r; bestMap = map; }
            }
